...
from necro.full_snapshots import rows as raw_rows
from necro.phase2_data import choice, example, reading
from necro.phase3_data import SourceIndex, coverage, historical, paired_public, take
from necro.source_isolation import signature
from necro.training_data import write_jsonl

ROOT = Path("data/phase3/instruction-v1")
...
def reading_pairs(rows):
    positive = (
        ...
        "Should the supplied question receive a yes response, judged only from the passage?",
    )
    negative = (
        ...
        "Should the supplied question receive a no response, judged only from the passage?",
    )
    result = []
...
def prepare():
    ...
    prior_paths = sorted(Path("data/phase3").glob("*/train.jsonl"))
    ...
    result = register(
        ...
        {"train": ROOT / "train.jsonl", "development": ROOT / "validation.jsonl"},
        {
            ...
            "training. Paired variants are associated samples, not independent sources. "
