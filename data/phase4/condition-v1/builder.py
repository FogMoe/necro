...
def instructions(fields, code, language, negated, style):
    a, b, gate = fields
    _, zh, en = OPS[code]
                ...
                for case, current in enumerate(variants):
                    ...
                    for language in ("en", "zh"):
                        for negated in (False, True):
                            style = styles[(i + op_index + case) % len(styles)]
                            row = example(
...
def prepare():
    ...
    prior = {canonical_request(r) for p in Path("data").rglob("*.jsonl") for r in read_examples(p)}
    new_rows = [
        r for rows in parts.values() for r in rows if r["source"] == "constructed-condition-v1"
    ]
