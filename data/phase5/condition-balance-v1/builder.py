...
def prepare(root):
    ...
    historical = {
        canonical_request(r) for path in Path("data").rglob("*.jsonl") for r in read_examples(path)
    }
    if any(
    ...
    audit = audit_balance(parts["train"])
    register(
        root,
        files,
        {
            "scope": "Semantic weighting repair; exposed cohorts are regression evidence; test is sealed until selection.",
