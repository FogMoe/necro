...
This record defines the prepared data and portable Linux GPU runner for a fresh LoRA initialized from the official post-trained `Qwen/Qwen3.5-0.8B`. A comparison against a pretraining-only Base checkpoint is deferred. Execution details belong to the [cloud run record](unified-cloud-run-2026-09-21.md).

## Design

The two [local condition-balance trials](condition-balance-repair-2026-09-21.md) failed their repair checks. Their partial gains support imbalance as a contributing factor. The historical adapter chain also makes later task proportions and accumulated exposures difficult to reason about. The next experiment replaces that chain with one multi-task recipe and a fresh rank-16 LoRA.
...
The runner profiles checkpointed microbatches before choosing a size at effective batch 16. The primary recipe uses two epochs and learning rate 1e-4. One lower-rate comparison at 5e-5 is registered; it can start only after primary assessment. Both start fresh, use seed 2026 and reject a changed base revision. The registered budget covers these two fresh-LoRA recipes.
