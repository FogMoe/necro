"""对共同起点、同配方 LoRA 做参数均值；不宣称等于完整增量矩阵的均值。"""
...
def average(left: Path, right: Path, output: Path, model_id):
    ...
    composition = {
        "method": "aligned LoRA parameter mean, not exact delta-matrix averaging",
        ...
        "notes": "No new gradient updates. Rank and merged model size are unchanged. "
        "This is an empirical candidate and must be evaluated before selection.",
    }
    ...
    contract = {
        **manifests[0],
        ...
        "objective_description": "Arithmetic mean of aligned LoRA factor parameters; no gradient updates.",
