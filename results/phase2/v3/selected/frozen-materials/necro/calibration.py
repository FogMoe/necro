...
def fit_primitive_temperatures(dataset, predictions_path, upper=4.0):
    ...
    for primitive, values in pairs.items():
        ...
        def objective(temperature, values=values):
            grouped = defaultdict(list)
            for p, target, family, group in values:
                grouped[family, group].append(-np.log(max(temper(p, temperature)[target], 1e-30)))
        ...
        temperature = min(grid, key=objective)
        result[primitive] = {
            "temperature": temperature,
            "before_nll": objective(1.0),
            "after_nll": objective(temperature),
    ...
    return {
        ...
        "primitives": result,
        "method": "per primitive; mean family NLL after averaging within source groups",
        "bounds": [0.5, upper],
