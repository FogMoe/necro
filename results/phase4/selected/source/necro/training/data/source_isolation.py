...
from necro.training.data.phase2_data import example, load
...
def unseen_retrieval(blocked, count=80, *, seed=20260921, namespace="strict-test", split="test"):
    ...
    qrels = [
        ...
        for row in load(f"scifact-{part}")
    ]
    for row in qrels:
        if row["score"] > 0:
    ...
    result = []
    for qid in qids:
        if "scifact-query/" + signature(queries[qid]) in blocked:
            continue
    ...
    if count is None:
        return result
    raise ValueError(f"独立文档的检索题数量不足：{len(result)} / {count}。")
