...
The final repair uses the complete condition matrix and retained task replay. Training history, failed candidates and recipe amendments are in the [repair process record]({{DOCS}}process/condition-repair-2026-09-21.md). The [frozen decision]({{EVIDENCE}}selection/decision.json) records development selection. Per-primitive Brier calibration uses a separate registered cohort: Choice {temperatures["choice"]:.9f}, Noul {temperatures["noul"]:.9f}, Score {temperatures["score"]:.9f}. Exact values are in [calibration.json]({{EVIDENCE}}selection/calibration.json).
