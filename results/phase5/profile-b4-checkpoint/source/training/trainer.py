...
def answer_loss(model, inputs, targets, candidate_ids=None, sample_weights=None):
    ...
    losses = []
    for i, candidates in enumerate(candidate_ids or [None] * len(targets)):
    ...
    if sample_weights is not None:
        weights = torch.as_tensor(sample_weights, device=losses.device, dtype=losses.dtype)
...
def train(
    ...
    config = {
        ...
        "training_weights": {"mean": weight_mean, "min": min(r["weight"] for r in encoded), "max": max(r["weight"] for r in encoded)},
...
def main():
    ...
    parser.add_argument("--profile-only", action="store_true", help="Measure forward/backward throughput without updating weights.")
