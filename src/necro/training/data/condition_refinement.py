...
def construct(revision=1):
    if revision == 3:
        partitions = construct(1)
        partitions["train"] = complete_condition_matrix()
        return partitions
...
def complete_condition_matrix():
    rng = random.Random(2026092109)
    rows = []
    for i, threshold in enumerate(rng.sample(range(17, 987), 16)):
        fields = FIELDS["train"][i % len(FIELDS["train"])]
        a, b, gate = fields
        style = (i + i // len(FIELDS["train"])) % 4
        step = 0.5 if i % 2 else 1
        group = f"condition-v3/train/{i}"
        for code, (fn, _, _) in OPS.items():
            ...
            for truth, delta in ((True, positive), (False, negative)):
                complete = {a: threshold + delta, b: threshold, gate: True}
                assert bool(fn(complete[a], complete[b])) == truth
                cases = [("complete", complete), ("disabled", {**complete, gate: False})]
                cases += [
                    (f"missing-{position}", {k: v for k, v in complete.items() if k != field})
                    ...
                    for language, negated in product(("en", "zh"), (False, True)):
                        row = example(
                            ...
                            copy.deepcopy(state),
                            {
                                "type": "noul",
                                "instructions": instructions(
                                    fields, code, language, negated, style, 2
                                ),
                            },
                        ...
                        rule_metadata(row)
                        rows.append(row)
    audit_condition_matrix(rows)
    return rows


def audit_condition_matrix(rows):
    counts = Counter()
    for row in rows:
        ...
        counts[
            (
                ...
                row["language"],
                meta["negated"],
                row["condition_matrix_case"],
                row["condition_relation_true"],
            )
        ] += 1
    required = set(
        product(
    ...
    if set(counts) != required or len(set(counts.values())) != 1:
        raise ValueError("Condition combinations must be complete and balanced")
    return {"combinations": len(counts), "examples_per_combination": min(counts.values())}


def prepare(revision=1):
    root = ROOT if revision == 1 else ROOT.with_name(f"condition-v{revision}")
    if root.exists():
    ...
    manifest = register(
        ...
        files,
        {
            ...
            "condition_matrix": audit_condition_matrix(
                [r for r in parts["train"] if r["source"] == "constructed-condition-v1"]
            )
            if revision == 3
            else None,
